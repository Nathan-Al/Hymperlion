...
function CreerSite($nom, $choix)
{
    ...
    require $manager_page_model;

    $fichier = array("vue"=>$racine_site.$nom."/Views"."/"."view-", "controller"=> $racine_site.$nom."/Controller"."/"."control-", "css"=> $racine_site.$nom."/Css"."/"."css-");
    $dossier = array("Views","Controller","Css","Model","Outil","Error","Medias-Site");
    $ok = 0;
    $do_all_the_work;
    ...
    if($choix)
    {
        ...
        {
            if(CopierDossier($racine_template."Default/",$racine_site.$nom."/"))
            {
    ...
    }else
    {
        ...
        if($ok=6)
        {
            if(CreeFichier($racine_site.$nom."/", "index.php") && CreeFichier($racine_site.$nom."/", ".htaccess") && CreePage($fichier,$nom))
            $do_all_the_work = true;
        }
    } 
